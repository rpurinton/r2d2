...
class Base
{
	...
	function __destruct()
	{
		if($this->mq_chan != null) $this->mq_chan->close();
		if($this->mq_conn != null) $this->mq_conn->close();
	}

	protected function rabbitConnect($config)
	{
		extract($config);
	...
	protected function publish($queue,$data)
	{
	...
	protected function discordClean($message)
	{
	...
	static function myReplace($search, $replace, $mixed)
	{
	...
	static function randId()
	{
	        ...
	        for ($i = 0; $i < 12; $i++)
	        {
	                if (rand(0, 1) === 1) $result .= strtoupper($r[array_rand($r)]);
	                else $result .= $r[array_rand($r)];
	        }
	        return $result;
	}
...
class Controller
{
	protected function getPids()
	{
		...
		foreach($ps as $line)
		{
			if(strpos($line,"grep") === false) $ps2[] = $line;
		}
		foreach($ps2 as $line)
		{
...
class Status Extends Controller
{
	...
	{
		$pids = $this->getPids();
		if(sizeof($pids) == 4) echo("R2D2 is running... (pids ".implode(" ",$pids).")\n");
		elseif(sizeof($pids)) echo("WARNING; R2D2 is HALF running... (pids ".implode(" ",$pids).")\n");
		else echo("R2D2 is stopped.\n");
	}
}
...
class Start Extends Controller
{
	...
	{
		$pids = $this->getPids();
		if(sizeof($pids)) die("ERROR: R2D2 is already running.  Not starting.\n");
...
class Main
{
	...
	function __construct()
	{
		...
		{
			$this->functions["worker$id"] = function($workerid) { new Worker($workerid); };
		...
		$this->functions["discord"] = function() { new DiscordClient; };
		...
		$this->functions["highviber"] = function() { new HighViberClient; };
...
class Worker Extends Base
{
	...
	protected function sqlConnect($config)
	{
		extract($config);
	...
	protected function sendReply($data, $message)
	{
		extract($data);
		switch($platform)
		{
	...
	protected function highviberSend($channel, $message)
	{
		sleep(1);
	...
	protected function logSql($userid, $username, $message)
	{
	        ...
	        if (isset($this->levels[$message_count])) return $this->levels[$message_count];
        	return 0;
	}

	protected function getLevel($messages)
	{
	        if ($messages < 4) return 0;
	        for ($i = 1; $i < 100; $i++) if ($messages >= $this->levels_reverse[$i] && $messages < $this->levels_reverse[$i + 1]) return $i;
	        return 100;
	}
		...
		$discord->on("ready", function (Discord $discord) {
		        $discord->on("raw", function ($data, Discord $discord) {
		                        ...
		                        $data["d"]["author"]["username"] != "HighViber" )
		                {
		                        ...
		                        if(strpos($text," ") !== false) $packet["vars"] = substr($text,strpos($text," ")+1);
		                        else $packet["vars"] = "";
...
class HighViberClient Extends Base
{
	...
	function processMessage($connection,$payload)
	{
	        ...
	        {
	                $data = json_decode($payload['data'], true);
	                if($data["user"]["first_name"] == "R2D2") return;
	                $packet["platform"] = "highviber";
	                $packet["socket_id"] = $this->socket_id;
